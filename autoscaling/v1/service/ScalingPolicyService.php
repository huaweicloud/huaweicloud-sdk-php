...
class ScalingPolicyService {
    ...
    public function createPolicy(array $req, ASV1Client $asClient)
    {
        $authAkSkRequest = new AuthAkSkRequest();
        $authAkSkRequest->setMethod(Constants::HTTP_METHOD_POST);
        $authAkSkRequest->setUri('/autoscaling-api/v1/'.$asClient->getConfig()['project_id'].'/scaling_policy');
    ...
    public function updatePolicy(array $req, ASV1Client $asClient)
    {
        $authAkSkRequest = new AuthAkSkRequest();
        ...
        $authAkSkRequest->setUri('/autoscaling-api/v1/'.$asClient->getConfig()['project_id'].'/scaling_policy/'.$req['scaling_policy_id']);
        $authAkSkRequest->setHost($asClient->getConfig()['endpoint']);
        $authAkSkRequest->setHeaders(array('Content-Type'=>'application/json'));
        if(empty($req['body'])){
            throw new Exception("Please build your request body");
    ...
    public function getPolicyList(array $req, ASV1Client $asClient)
    {
        $authAkSkRequest = new AuthAkSkRequest();
        ...
        $authAkSkRequest->setUri('/autoscaling-api/v1/'.$asClient->getConfig()['project_id'].'/scaling_policy/'. $req['scaling_group_id']. '/list');
        $authAkSkRequest->setHost($asClient->getConfig()['endpoint']);
        if(!empty($req['query'])){
    ...
    public function actionPolicy(array $req, ASV1Client $asClient)
    {
        $authAkSkRequest = new AuthAkSkRequest();
        $authAkSkRequest->setMethod(Constants::HTTP_METHOD_POST);
        if(empty($req['scaling_policy_id'])){
        ...
        $authAkSkRequest->setUri('/autoscaling-api/v1/'.$asClient->getConfig()['project_id'].'/scaling_policy/'. $req['scaling_policy_id']. '/action');
        $authAkSkRequest->setHost($asClient->getConfig()['endpoint']);
        $authAkSkRequest->setHeaders(array('Content-Type'=>'application/json'));
        if(empty($req['body'])){
            throw new Exception("Please build your request body");
